...
// 주소 검색 버튼 클릭 시(myPage-info.html 외에는 문제가 발생하지 않도록 예외처리 해둔 부분!)
...
if(updateInfo != null) {
    ...
    updateInfo.addEventListener("submit", async (e) => {
        ...
        if(currentNickname !== memberNickname.value) {
            ...
            //          -> "이 작업이 끝날때까지 기다려주세요."
            const resp = await fetch("/member/checkNickname?memberNinkname="+ memberNickname.value);
...
imageInput.addEventListener("change", () => {
    ...
    if(file) { // 파일이 선택된 경우
        if(file.size <= MAX_SIZE){ // 파일 크기가 허용범위 이내인 경우
            ...
            // (alert 창은 띄었지만 이미 선택된 큰 사이즈 파일을 비우는건 따로 해야함)
            ...
            if(previousFile) {
                ...
                // 이전 파일을 추가 해두기 : DataTransfer에 File 객체를 추가
